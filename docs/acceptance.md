...
| 所有する検証 pool 内の burst 試験 | 実装済み（macOS、process provider） / 未検証（Lima VM の Firecracker） | `scripts/e2e/burst.sh` → `docs/evidence/20260917T051238Z-burst-process/`（9/9 PASS）。KVM 上の burst は、検証 VM を jailer / cgroup の計測（PLT-4622）が使用中だったため実行していない |
