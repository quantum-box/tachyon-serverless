...
| └ security regression group | | | | `scripts/ci/security-regression.sh`: `scripts/ci/security-regression.list` の 142 テスト（tenant 認可 30、reuse key 9、lease / epoch 26、deadline 12、egress（起動ゲートの順序・allowlist・nftables 規則）15、資源上限 50（うち PLT-4634 の admission・quota・queue 上限・breaker・placement 18、PLT-4638 の queue 満杯・object の size / quota 2、PLT-4635 の scale-down 判定・min_ready・非振動 7）。tenant 認可のうち 5 は PLT-4638 の object の tenant 境界と queue の匿名接続拒否、2 は PLT-4635 の削除後の受付・待機・再送の拒否。PLT-4635 は ほかに reuse key 2（alias 切替・secret rotate の drain）、lease / epoch 3（sweep と claim の race、先行起動の pool 入り）、deadline 1（drain timeout））を `--exact` で実行し、**list にあるテストが実行結果に現れなければ失敗**（rename・削除・`#[ignore]` で coverage が黙って落ちない） | |
