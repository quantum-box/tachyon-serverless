...
| cron / webhook trigger | `scripts/queue/triggers-e2e.sh`（証跡は既定で `target/queue/triggers-e2e-<UTC>/`、`--evidence DIR` で変更。`curl` `jq` `openssl` `python3` が要る） |
