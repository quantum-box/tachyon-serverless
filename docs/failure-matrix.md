...
| 4b | `db_locked_past_lease` | 同じ lock を 12 s（lease 6 s + skew を超える） | 応答は成功・503 `Host.StoreUnavailable`・fenced の 503（`provider_unavailable`）のいずれか。**gateway は heartbeat の延長を拒否されて自分を fence し、再起動するまで 503 のまま**（`recovery.fenced_itself_after_losing_its_lease`、既知の制約 §6.2）。再起動で復帰し、受付済みはすべて収束 |
...
| 項目 | 内容 | 影響と扱い |
|---|---|---|
| store 停止が lease を超えると gateway は自分を fence する | `db_locked_past_lease`: heartbeat が store に書けないまま `lease_ttl + max_clock_skew`（既定 30 s + 2 s）を過ぎると、復旧後の延長が拒否されて dispatcher は fenced になり、再起動まで `/readyz` 503・新規 invoke 503、非同期 run は `Host.DispatcherFenced` で数えない先送りを続ける。回収する他の gateway がいなくても同じ | ADR-0003（PLT-4631）「選んだこと」5 の再登録しない設計の帰結。安全側（二重実行しない）だが可用性は落ちる。運用で再起動する（supervisor の liveness に `dispatcher.fenced` を使う）か、誰にも回収されていない lease の再取得を後続で設計する |
...
| 他の SQLite も同じ mutex の形 | `usage/ledger.rs`、`durable/sqlite_queue.rs` の connection mutex は時間で打ち切っていない | usage journal / ledger（別 file）の lock は試していない。同じ修正が当てはまる |
