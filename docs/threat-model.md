...
| A5 ledger（Function / Revision / Alias / Invocation / Attempt / Environment / Lease / UsageEvent） | `<data_dir>/state.db`（埋め込み SQLite。invocation log は別の `<data_dir>/logs/logs.db`、ADR-0018）。UsageEvent は `<data_dir>/usage/journal.db`（未回収）と `<data_dir>/usage/ledger.db`（回収済み、PLT-4642） | 偽の結果による上書き、guest 申告に基づく計測、usage の改竄・削除・二重計上・他 tenant への漏洩 |
...
| `GET /v1/usage`（PLT-4642、token の tenant の仮料金報告） | 200（A の分だけ） | 200（B の分だけ。`function_id` に A の function を指定しても空） | 403 | 401 |
...
| log / invocation | 2000 行、1 MiB | 超過分を捨て `LogsResponse.dropped = true`、`[tachyon] ` marker 1 行 |
| log / attempt | `[logs] max_lines_per_attempt` / `max_bytes_per_attempt`（既定は invocation と同じ） | 同上 |
| log の保存全体（`logs.db`） | `[logs] max_total_bytes` 1 GiB、`retention_seconds` 7 日、writer queue 20000 行 / 16 MiB | 期限切れと terminal の古い invocation から削除。queue 満杯・store 不可は捨てて数える（invoke は止めない）。実行中の invocation の log は総量上限で消さない（ADR-0018 決定 7） |
...
| T03 | secret 値がログ・応答・state に混入 | `SecretValue` の `Debug` redact、`HelloAck.env` にだけ載せる、`HelloAck` を log しない、`BootEvidence.details` に secret 禁止 | 実装ミス。host 側は `crates/application/tests/pipeline.rs::secret_values_never_reach_host_logs`（TRACE で捕捉した host log と invocation log に値が無いこと）と `secret_values_reach_the_log_store_only_when_user_code_prints_them`（`logs.db` と WAL の bytes に値が無く、user code が自分で出力したときだけ入ることを陽性対照で確認）で確認。bridge 側の同等テストは PLT-4623 |
...
| T09 | log 洪水による retention 破壊・disk 枯渇 | invocation / attempt ごとの行数 / bytes 上限、`dropped` と marker で観測。`logs.db` は保持期限と総量上限、writer queue も上限付きで、書き込みが追いつかなければ捨てて数える（invoke を待たせない。`crates/application/src/logs/tests.rs::a_full_queue_drops_and_counts_without_blocking_the_caller`） | 実行中の invocation の log は総量上限で消さないので、同時実行数 × invocation 上限までは上限を超えうる。SQLite の overhead は `max_total_bytes` に含まない |
...
| T34 | `GET /metrics` から他 tenant の tenant id・revision id・待ち時間・環境の資源使用量を読む / tenant の token や operator role で全 tenant の観測を得る / 大量の tenant・revision で metrics の label cardinality を爆発させ gateway や Prometheus を圧迫する / 負荷試験ハーネスで本番・外部 host に負荷を送る（PLT-4637） | `/metrics` は `[metrics] bearer_token`（16 bytes 以上、定数時間比較）でだけ応答し、未設定なら 404、tenant の token（operator role を含む）は 401 で本文に tenant 情報を含まない。設定検証で tenant token・`internal_token` と同じ値を拒否。`GET /v1/capacity` には node 全体の `reuse` しか足さない。tenant / revision / environment の系列は `[metrics] max_*_series` で上限を持ち、超えた分は `_other` に合算。scrape で provider を読む環境は最大 1024。`tsls-load` は loopback と明示した lab host だけを送り先にし（名前解決・redirect なし）、宣言した同時数・件数・期間と compile 時の天井を超える計画は送らない（`apps/gateway/tests/gateway_integration.rs::metrics_require_the_operator_credential_and_never_leak_tenants_without_it`、`config::tests::metrics_section_is_off_without_a_token_and_refuses_weak_or_tenant_tokens`、`metrics::tests::tenant_and_revision_series_beyond_the_caps_fold_into_other`、`apps/load` の `limits::tests::*`） | `bearer_token` が漏れれば全 tenant の id・revision id・待ち時間・使用量が読める（通信路は平文 HTTP、§14-3）。別 listener ではないので、外部に公開する構成では reverse proxy で `/metrics` を塞ぐ必要がある。`_other` への畳み込みで個々の tenant の starvation が見えなくなりうる（`tsls_metrics_series_truncated` で分かる）。scrape の頻度自体は制限していない。`--lab-host` に何を渡すかは運用者の判断 |
| T36 | usage の journal / ledger の行を書き換える・消す・挿入して利用量を減らす（または他 tenant に付け替える）、collector の crash や再送で二重計上する（PLT-4642） | journal と ledger は `<data_dir>/usage/` の別ファイルで mode 0600。journal の各行は前の行との hash chain（`sha256(prev ‖ body)`）を持ち、collector は cursor から再計算して合わない行で止まる（`usage::tests::a_tampered_journal_row_stops_collection`）。ledger は `event_id` 主キーで、再送・crash 後の再配送・遅着は 1 行（`a_crash_between_the_ledger_commit_and_the_cursor_never_double_counts`、E2E の SIGKILL crash point）。ledger の読み出しは tenant 列と本文の tenant の両方を照合する | host の root（= gateway を動かす者）は journal 全体と chain を作り直せる。ledger 自体に chain は無い。prototype の ledger は gateway ごとの local file で、複製・署名・外部 anchoring が無い（§14-15） |
| T37 | 計測できない状態で実行を続けて利用量が欠ける / 推測値で請求する、guest 申告で課金を操作する、他 tenant の利用量を読む、仮料金を請求書と取り違える（PLT-4642） | journal が満杯・停止なら新規 invoke を台帳に書く前に 503 `usage_journal_full`（fail closed、`accept_unmetered` は dev profile だけ）。受付済みの実行の event は headroom で書き切り、書けなかった分は tenant ごとの `unjournaled` に数える。rating は `host_measured` / `provider_reported` だけを読み、`unknown` / `guest_reported` は 0 で `unmetered` に出す（`guest_reported_times_never_reach_a_charge`）。`GET /v1/usage` は token の tenant だけ・operator は 403（`usage_reports_are_tenant_scoped`、gateway `usage_report_is_provisional_tenant_scoped_and_fails_closed`）。応答は常に `provisional` / `not_an_invoice` / `billing_enabled = false`、`[usage.billing] enabled = true` は設定エラー | headroom を超える同時実行があれば受付済みの event も `unjournaled` になりうる（件数は残る）。dispatch 済みで gateway が kill -9 された attempt の `AttemptSettled` は出ない（その区間は利用量に無い）。非同期 invoke の受付は journal を見ない（実行する PLT-4640 で `admit` を呼ぶ） |
| T45 | webhook の偽造・再送: secret を知らない者が関数を起動する、盗聴した署名済み request を再送・event id だけ変えて再送する、古い request を後から流す、存在しない / 他 tenant の trigger を探る（PLT-4641） | HMAC-SHA256（`"{timestamp}.{body}"`、key = 256 bit の生成 secret）を定数時間で比較し、timestamp は tolerance（既定 300 s、上限 3600 s）の外を 401。どちらも **永続化の前**。event id と検証済み署名の digest を fire 行の一意キーにし、同じ event id・同じ署名は同じ invocation を返す（dedup は tolerance の 2 倍以上保持）。trigger id 不明・削除済み・cron は同じ 404、無効は署名済み request にだけ 410。invocation の実行は trigger の tenant の権限で、fire ごとに設定 cache で tenant の認可を確認する（`services::triggers::tests::{webhook_refusals_happen_before_any_durable_write, a_replayed_webhook_answers_the_same_invocation_and_creates_nothing, disabled_and_deleted_webhook_triggers_answer_without_leaking_existence}`、`services::triggers::webhook::tests::signature_fixtures_verify_and_refuse`、`scripts/queue/triggers-e2e.sh`） | 署名は event id header を覆わない（署名 digest の dedup で補う。代償として同じ秒・同じ body の別 event は 2 件目が再送扱い）。dedup の保持期間を過ぎた event id を署名し直して送れば新しい invocation になる。401 と 404 の違いで「その id の webhook trigger が存在する」ことは分かる（id は 80 bit の乱数を含む）。TLS は gateway の前段（ADR-0004）に依存し、平文で盗聴されれば tolerance 内の再送は dedup でしか止まらない |
| T46 | trigger 経由の資源枯渇と secret の漏洩: 巨大 body での memory 消費、毎秒の cron や大量 webhook による受付の占有、再起動のたびの二重発火、secret が API・log・台帳・証跡に残る（PLT-4641） | body は `Content-Length` で読まずに、無ければ読みながら trigger の `max_body_bytes` で打ち切る（413、`apps/gateway/tests/triggers_http.rs::webhook_http_refuses_before_storing_and_dedups_resends`）。fire は invokeAsync と同じ outbox の上限（T33）に掛かり、function ごとの trigger 数は `max_triggers_per_function`。missed run は `max_catchup_seconds` と `max_run_all` で有界。同じ予定時刻は fire 行の主キーで 1 回（`a_restart_on_the_same_data_dir_never_fires_a_scheduled_time_twice`、`two_schedulers_on_one_ledger_fire_each_scheduled_time_once`）。secret は作成 / rotate の応答だけ（`no-store`）、台帳には AES-256-GCM（AAD = trigger + tenant）で封じ、JSON 本文・log・`Debug` に出さず、削除で消す（`a_webhook_secret_is_shown_once_and_never_stored_or_returned_in_plain`、`trigger_http_crud_never_returns_the_secret_and_never_crosses_a_tenant`） | 秒単位の cron に最小間隔は無く、outbox の上限は全 tenant 共通（1 tenant の trigger が他 tenant の受付を 429 にしうる）。封じる鍵は `state.db` と同じ host の file / 環境変数で、host を取られれば secret も取られる。rotate に猶予期間は無い（sender 側の切替と同時に旧 secret が失効する） |
| T40 | 他 tenant の dead letter を読む・redrive する、`redrive` role の無い主体（`invoke` だけの token、operator）が再実行する、redrive で他 function・他 tenant の Revision や入力を指定して別の tenant の仕事を動かす / 入力を複製する（PLT-4640） | 読み取りは `invoke`、redrive は `invoke` と `redrive` の両 role（`authz::require_redrive`）。dead letter は tenant で絞って読み、他 tenant の id は存在しない id と同じ 404、他 tenant の function の一覧は空。redrive は role の検査の後に tenant を確認し、Revision は呼び出し元の principal として設定 cache で解決（他 function・他 tenant は 404 / 400）、台帳のトランザクションでも invocation・入力・outbox・redrive 記録がすべて dead letter と同じ tenant と function であることを確かめて拒否する。入力は同じ tenant の同じ本文 / object の参照だけで、`object_refs` の attach も tenant を検査する。redrive は監査記録（subject、理由、時刻、元 invocation、dead letter）と `redriven` を同じトランザクションで書き、2 回目は 409（`crates/application/src/services/invoke_async/dispatch_tests.rs::{redrive_is_authorized_and_never_crosses_a_tenant, a_redrive_creates_an_audited_invocation_on_the_pinned_revision_and_input}`、`apps/gateway/tests/gateway_integration.rs::dead_letters_and_redrive_are_authorized_audited_and_tenant_scoped`、E2E `redrive.*`） | `redrive` role は tenant 単位で、function ごとの権限は無い。監査記録は台帳の同じ file にあり、台帳を書ける者は改竄できる（署名・外部転送なし）。`requested_by` は token の subject で、subject の一意性は設定に依存する |
| T41 | decode できない / 偽造した event（他 tenant の invocation id を別 tenant で routing、version 違い）で dispatcher を無限に再配送させる、他 tenant の invocation を実行させる、poison で dead letter を膨らませる（PLT-4640） | envelope は message id・generation・routing tenant・台帳の tenant / function / revision / digest と照合し、合わなければ「この tenant の invocation は無い」として poison の dead letter（invocation に結ばない、routing tenant・message id・sequence・payload の digest と size だけ）を記録して `term`。同じ message（id と sequence）は `origin_key` の unique で 1 件。記録に失敗したら NAK して後でやり直す。panic / crash を繰り返す handler は poison ではなく `Crash` の retry として `max_attempts` で `attempts_exhausted` に収束する（`crates/application/src/services/invoke_async/dispatch_tests.rs::poison_events_are_dead_lettered_once_and_never_redelivered`、E2E `poison.*`） | queue に publish できる者（gateway の credential、T27）は poison の dead letter を作れる（件数の上限・保持期限は無い）。poison の dead letter は routing tenant の名前で記録され、その tenant の `GET /v1/dead-letters/{id}` で読める |
| T42 | 失敗し続ける依存先への retry storm、retry が node の容量と同期 invoke の枠を食い潰す、broker の配送回数が kill -9 で数え直されて retry が止まらない（PLT-4640） | retry の回数・期限は台帳（`async_dispatch.attempts`、`min(accepted_at + max_event_age, queue_deadline)`）が数え、broker の `max_deliver` に頼らない。backoff は full jitter の指数。function ごと・window ごとの retry budget を超えた retry は次の window へ先送り。非同期の並列は `workers`（既定 `max_concurrency` の半分）で、容量待ちは `admission_wait_ms` で打ち切って先送りし、同期 invoke の前で長く並ばない。先送りは数えないが期限で `expired` に収束する（`retry::tests::*`、`crates/application/src/services/invoke_async/dispatch_tests.rs::{retries_over_the_budget_are_deferred_without_counting, a_retryable_failure_is_retried_with_backoff_and_then_succeeds, exhausted_attempts_are_dead_lettered_with_the_last_error, an_event_past_its_maximum_age_is_dead_lettered_without_running}`） | retry budget は gateway process ごとで、複数 gateway では合計が gateway 数倍になる。budget・workers は tenant ごとではなく、1 tenant の retry が同じ function の他の invocation と node の非同期の枠を使う（admission の tenant quota は効く）。予約中の retry は outbox の件数上限に数えられ、受付の 429 `backlog` を早める |
| T43 | ACK の喪失・dispatcher の停止・重複配送で、handler が 2 回実行され二重の terminal / 矛盾した結果が残る、あるいは reclaim / 再起動が retry の機会を奪って受け付けた処理を失う（PLT-4640） | 決定は台帳: claim（`async_dispatch` の CAS）で 1 run、settle は claim で fence した 1 トランザクション（terminal / 次の予約 / dead letter）、ACK は commit の後。terminal の invocation の再配送は実行せず ACK。claim が切れた run の遅れた結果は fence で捨てる。reclaim と restart reconcile は非同期 invocation を terminal にせず attempt だけを settle し、reaper が claim 切れを次の試行に回す。4 つの failpoint（claim 後、副作用後・commit 前、retry commit 前、commit 後・ACK 前）それぞれで停止 → 再起動の収束を確認（`crates/application/src/services/invoke_async/dispatch_tests.rs::{an_ack_lost_after_the_terminal_commit_is_not_run_again, a_consumer_that_stops_mid_run_is_taken_over_after_its_claim_expires, a_crash_between_the_side_effect_and_the_commit_runs_the_handler_again, a_crash_before_the_retry_commit_is_retried_after_the_claim_expires, concurrent_duplicate_deliveries_run_the_handler_once, a_settle_from_a_claim_that_was_taken_over_is_refused}`、E2E `crash.*`・`converge.*`） | **実行は at-least-once**: 副作用の後・settle の前に止まれば handler はもう一度走る（§14-16、`docs/api.md` §5.6.3）。claim が生きている間に SIGSTOP などで止まった run は、claim 期限の後に別の run と並行しうる（結果は 1 つだけ記録される）。claim 期限の判定は wall-clock（§14-8 と同じ） |
| T44 | dead letter が入力（inline 本文・object）を無期限に保持して漏らす / GC が redrive 前の入力を消して redrive が別の入力を読む（PLT-4640） | dead letter は入力を複製せず、`invocation_inputs` と `object_refs` の既存の参照を指す。`open` の dead letter が参照する object は GC が消さない（`claim_for_collection`）。redrive は digest を引き継ぎ、実行時に入力を台帳の tenant scope で読んで digest を検証する（不一致は `Host.InputCorrupt` の dead letter）。object が tombstone 済みなら redrive は 409（`crates/application/src/services/invoke_async/dispatch_tests.rs::a_redrive_creates_an_audited_invocation_on_the_pinned_revision_and_input`、`a_non_retryable_error_is_dead_lettered_immediately`） | dead letter・redrive 記録・inline 入力に保持期限・削除 API が無く、台帳に平文で残り続ける（T32 と同じ）。`redriven` の dead letter が参照する object は GC の保護から外れ、TTL が付いていれば期限で消える（その後の redrive は無い） |
| T50 | 並列の invoke（同じ gateway の多数の request、同じ `data_dir` の 2 つ目の gateway）で予約の読みと書きの隙間を突き、hard limit を超えて実行させる / 予算 store の行を書き換えて残高を増やす（PLT-4643） | 予約は `<data_dir>/usage/budget.db` の `BEGIN IMMEDIATE` 1 トランザクションで「期間の totals を読む → 上限検査 → 行と totals を書く」（SQLite の write lock で直列化）。16 thread × 別接続で上限ちょうどの件数だけが通る（`budget::tests::concurrent_reservations_on_separate_connections_never_exceed_the_limit`）、12 並列の invoke で 4 件だけ（`tests/budget.rs::parallel_invocations_never_reserve_beyond_the_hard_limit_and_the_excess_returns`）、E2E 3a / 3b（8 並列で 3 件）。file は mode 0600 | host の root は store を書き換えられる（複製・署名・chain なし、§14-18）。`data_dir` を共有しない gateway 同士は予算を共有しないので、同じ tenant を別 `data_dir` の gateway に振り分ければ上限はそれぞれに効く |
| T51 | 予約・精算・返却・失効の呼び出しを重複・並べ替え（collector の再配送、driver と失効の競合、同じ run id での再予約）して、二重に返却させる・残高を負にする・同じ予約を 2 回精算する（PLT-4643） | 状態機械 `reserved → settled \| released \| expired` の遷移はすべて `UPDATE … WHERE state = 'reserved'` で、重複は何も変えない。予約は run id が主キーで冪等。release は run が終わっていない行だけ、expire は終わりを報告していない行だけ。totals は行と同じトランザクションで動き、列の `CHECK (… >= 0)` で負になる更新は commit されない。property test で 300 round × 重複 1〜3 回の任意の順序の後に totals = 行の和、各予約の遷移は高々 1 回（`budget::tests::property_duplicate_and_interleaved_transitions_never_break_the_balance`、`duplicates_are_no_ops_and_terminal_states_never_move`、`tests/budget.rs::a_run_that_never_reports_back_expires_to_an_unmetered_hold`） | 精算は ledger の attempt id の列に依存する。driver が `AttemptSettled` を出した attempt を列に入れ損なう不具合があれば、その分は精算に入らない（試験は panic 時に `complete = false` として hold を残すことだけを確かめている） |
| T52 | 計測できない・予算が分からない状態で実行を続けさせる: collector を止める（ledger 停止、journal の integrity 停止）、予算の配信を止める・古い generation を再送する、価格表を差し替える、予算 store を壊す（PLT-4643） | fail closed: 予算 entry の未配信・tombstone・auth lease 切れ・価格表 / 通貨の不一致 → 503 `Host.BudgetUnknown`、終わった run の精算が `max_unsettled_age_seconds` を越えて遅れる → `Host.BudgetUnknown`（`/readyz` 503、`collector_stalled`）、store を開けない・書けない → `Host.BudgetStoreUnavailable`。古い generation は ADR-0007 の規則で予算を巻き戻さない。壊れた budget file は publication に使わず最後の有効な予算を配り続ける。開始済みの run は止めず deadline まで追跡、報告の無い run は deadline + grace で最大額を hold（`tests/budget.rs::{a_stalled_collector_fails_closed_and_admission_resumes_after_it_catches_up, an_unavailable_budget_store_refuses_with_its_own_error_type, budgets_are_tenant_isolated_and_an_undelivered_budget_fails_closed}`、gateway `a_data_plane_enforces_delivered_budgets_and_reports_them_per_tenant`、E2E 7a〜8・10） | collector の停止を検知するまでの間（`max_unsettled_age_seconds`）と、停止の間に開始済みだった run の実行は続く（予約は最大額のままなので上限は守られる）。collector 停止は全 tenant の受付を止める（可用性より正しさ）。dev profile の `collector.pause` file は production profile では無視する |
| T53 | 予約額より多く使って上限を回避する（長い handler、大きな response、retry、guest 申告の操作）/ 計測を欠けさせて安く精算させる（PLT-4643） | 予約は PLT-4642 の rating と同じ丸めで、timeout・初期化 timeout・handshake・cancel grace・要求 vCPU / memory・課金区間・request × 2 + 最大 response size・invocation 1 から上限を出す（property test で warm 未配達 + cold retry を含む任意の実行の rating 以上、`budget::tests::property_the_reservation_covers_every_rating_within_the_bounds`）。上限の分からない区間（`teardown_ms` / `idle_pooled_ms`）を課金する価格表は `[budget] enabled` で設定エラー。rating は guest 申告を読まない。計測が欠けた・unknown の run は予約との差額を unmetered hold に残し、計測値より下に返さない。実測が予約を超えたら全額精算し `overrun_micros` に出す | 最大料金の前提（handler 時間は timeout + cancel grace 以下、attempt は run の window の中で直列）が崩れる不具合があれば overrun になり、確約は上限を overrun 分だけ超える（数えるが止めない）。host の原価（idle、teardown、attempt の無い boot）は予算の対象外 |
| T54 | 他 tenant の予算・残高・拒否数・alert を読む、他 tenant の上限を使い切って巻き込む、`/readyz` / `/metrics` から tenant の予算を読む（PLT-4643） | `GET /v1/budget` は token の tenant だけ（`invoke` role、operator role は 403、tenant id は principal からだけ取る）。totals と予約は tenant 列で分かれ、1 tenant の hard limit・拒否は他 tenant に効かない（`tests/budget.rs::budgets_are_tenant_isolated_and_an_undelivered_budget_fails_closed`、gateway `a_data_plane_enforces_delivered_budgets_and_reports_them_per_tenant`、E2E 10）。`/readyz` の `budget` は tenant を含まない。tenant label の予算 metrics は `[metrics] bearer_token` の operator credential だけ（T34）で、系列数に上限 | collector の停止・store の停止は全 tenant の受付を止める（1 tenant の大量 invoke が journal を満たして collector を遅らせれば、他 tenant も巻き込まれうる。tenant ごとの持ち分は無い）。metrics の上限を超えた tenant は系列に出ない（`_other` への畳み込みは無い） |
| T55 | snapshot の取り違え・持ち出し: 他 tenant の snapshot、別 revision / 別 host（CPU・KVM・kernel・VMM・rootfs）/ 別 device model 用の snapshot、改ざん・偽造した manifest を load させて、他者の初期化済み memory を実行させる・guest を壊す（PLT-4653、X1） | manifest（`crates/domain/src/snapshot.rs`）に tenant・function・revision + spec digest・provider / VMM / kernel / rootfs の sha256・bridge protocol version・jailer / cgroup mode・host kernel・CPU model / flags hash・KVM capability hash・drive / vsock / doorbell / NIC 数・memory・vCPU・scratch・egress と allowlist digest・鍵世代・secret 世代・lifecycle version・checkpoint phase・4 artifact の sha256 を固定し、canonical JSON に HMAC-SHA256 で署名。load 前に署名・digest・canonical 形を検証し（`a_tampered_or_foreign_manifest_does_not_verify`、`a_non_canonical_manifest_is_refused_even_when_signed`）、`check_compatibility` が 1 field でも違えば拒否、他 tenant は理由を 1 つ（`tenant_mismatch`）だけ返して他の比較結果を出さない（`property_any_single_field_change_is_incompatible_with_its_reason`、`property_another_tenant_is_always_refused_alone`、`tests/restore.rs::{another_tenants_snapshot_is_refused, a_profile_mismatch_is_refused, a_new_revision_does_not_use_the_previous_revisions_snapshot}`） | 署名鍵と暗号鍵は gateway host の file / 環境変数で、host の root は manifest を作り直せる。CPU の同一性は cpuinfo の hash で、microcode 以外の firmware 差や nested virtualization の下の物理 CPU の差は見えない |
| T56 | 壊れた artifact・差し替え: memory / vmstate / scratch / function drive の 1 byte 変更、切り詰め、chunk の入れ替え、他 tenant・他 snapshot の暗号文の流用（PLT-4653） | 保存時は 4 MiB chunk ごとに AES-256-GCM、AAD に tenant・snapshot・artifact 名・chunk 番号・最終 chunk flag（`snapshot/store.rs::corrupted_or_foreign_artifacts_are_refused`）。load の直前に clone が使う平文 4 file の sha256 を manifest と照合し、違えば `artifact_corrupted` で拒否して snapshot を `quarantined` にする（隔離は後の状態変更で解除されない。`a_corrupted_artifact_is_refused_and_quarantined`、`a_corrupted_sealed_artifact_is_refused`、KVM: `scripts/x1/clone-e2e.sh` の corrupt-require / corrupt-prefer）。Firecracker 自身は vmstate の CRC しか見ない（ADR-0015） | 検証と load の間の TOCTOU: 平文は root 専用 directory（0700、memory / vmstate は 0640 root:<jail gid>）にあり、root だけが差し替えられる。検証は 256 MiB を毎回読む（restore の時間に入る） |
| T57 | snapshot に秘密・顧客データが入る / clone 間で漏れる: 本番 secret や顧客の実行済みデータを含む memory が clone に配られる、clone 同士が scratch や identity を共有する、restore した copy が cold boot なのに restored と記録される（PLT-4653） | snapshot と restore は `restore.synthetic_init_sample = true`・secret binding なし・egress `none` の revision だけ（revision 作成と snapshot 作成の両方で拒否、`snapshots_are_refused_for_secrets_and_non_synthetic_revisions`）。source の `HelloAck.env` に secret を入れず、guest が `CheckpointWaiting{phase = checkpoint, after_restore_ran = false}` を報告した後にだけ snapshot を取る（Ready になった source は拒否、`a_source_that_does_not_hold_is_not_snapshotted`）。source は resume せず terminate。clone ごとに jail・cgroup・vsock UDS・scratch の copy・環境 id・instance id（`Restore` frame で restore 後に渡す）。restored と数えるのは最初の frame が source と同じ boot id の `Reconnect` のときだけで、`Hello` は `cold_boot_detected`（`a_cold_boot_on_a_clone_is_never_counted_as_restored`、`bridge_session::restore::tests::a_cold_boot_on_a_restore_is_refused`） | 「synthetic」は operator の申告で、中身を検査しない。同じ snapshot の clone は memory file を MAP_PRIVATE で page cache 共有する（同一 tenant 内の side channel、Kata の VM template と同じ警告）。user space の PRNG・一意 ID は function が after_restore で作り直す規約（PLT-4651）に依存する |
| T58 | 失効・期限切れ・鍵 rotation 後の snapshot の再利用、`require` が黙って cold になる / `prefer` の cold が restored に見える（PLT-4653） | catalog の状態 `active` 以外は load しない（`revoked` / `quarantined` / `expired`）。`expires_at` 以降は拒否して `expired` にする。暗号鍵・署名鍵・secret 世代が変われば `revoked`（stale）。`POST .../snapshots/{id}/revoke`。`require` は理由の code 付きで `Host.RestoreRequiredUnavailable`、`prefer` は `start_kind = cold` と `restore_fallback` を evidence に残す（`revoked_and_expired_snapshots_are_refused`、`a_rotated_key_makes_the_snapshot_stale`、`require_without_a_snapshot_fails_explicitly_and_boots_nothing`、`prefer_without_a_snapshot_starts_cold_and_records_why`） | 状態は gateway ごとの file catalog で、複数 gateway 間の失効伝播は無い。期限と失効は次の restore 計画の時点で判定し、実行中の clone は止めない |
| T59 | restore frame の悪用・version 混在: 古い bridge に v3 frame を送る、snapshot されていない guest に `Restore` で identity を注入する、restore した guest の時計が古いまま deadline や token の期限を判断する（PLT-4653） | `PROTOCOL_VERSION` 3、host は 2〜3 を受け入れ guest の version で話す（v2 には hold も restore frame も送らない、`only_protocol_versions_2_and_3_are_accepted`、`a_snapshot_needs_a_version_3_guest_that_holds`）。bridge は feature `experimental-restore` の build だけが v3。restore link は hold を求められ再接続した後の `Restore` だけを受け付ける（`restore_link::tests::a_restore_without_a_reconnect_is_ignored`）。`Restore.host_now_ms` で `CLOCK_REALTIME` を合わせてから `continue` に答える。deadline は従来どおり `remaining_ms` と host 側の強制 | 時計合わせは host の wall clock を信頼する（guest の monotonic clock は snapshot 時点から続く）。doorbell port（5001）は guest の listen socket で、同じ VMM の host 側 UDS に接続できる者（root / jail uid）は再接続を誘発できる |
...
15. **usage の ledger は local で、請求の根拠にはまだならない（PLT-4642）。** journal と ledger は gateway の host の SQLite で複製も署名も無く、root は書き換えられる（T36）。環境寿命（原価）は wall clock の差で、時計が飛ぶと狂う（利用量と仮料金は monotonic なので変わらない）。価格表の数値は仮置き。host の CPU / memory は terminate 直前の `environment_stats`（PLT-4637）の値で、Firecracker 実機では確かめていない。
17. **trigger（PLT-4641）。** webhook の署名は timestamp と body だけを覆い、event id header を覆わない（署名 digest の一意 index で再送を止めるが、同じ秒・同じ body の別 event は再送扱いになる）。event id の dedup は `webhook_dedup_retention_seconds` を過ぎると忘れる。secret を封じる鍵は `state.db` と同じ host の file / 環境変数にあり、rotate に猶予期間が無い。cron は gateway の時計で判定し、scheduler lease の TTL の間は owner の停止に気付かず、その間の時刻は missed-run policy に従って実行されないことがある。秒単位の cron に最小間隔は無く、trigger による受付も outbox の全 tenant 共通の上限（§14-13）を使う。trigger の fire は tenant の `invoke` 権限で動き、作成した token の revoke では止まらない（tenant が grant から消えれば止まる。trigger 自体は `deploy` の token で無効化・削除する）。data plane では trigger を扱わない（ADR-0014 §6）。
19. **snapshot / clone は実験経路（PLT-4653、X1）。** 既定では何も起きない（`[snapshots] enabled` と revision の `restore.policy` と provider feature `experimental-restore` がそろったときだけ）。Firecracker の capability は `Unverified` で、使うには `[snapshots] allow_unverified` の計測 run が要る。実測は aarch64 nested virtualization の 1 host・同一 CPU だけで、x86_64・bare metal・host 間の移送は未検証。egress `restricted` / `public-web` の clone（tap・IP・MAC の付け直し）は未実装で拒否する。restore 後に secret を渡す経路が無いので secret binding のある revision は対象外。平文の snapshot file は restore 用に host の `<workdir>/_snapshots/` に残り（root 専用）、暗号化されているのは data_dir の保管分だけ。catalog・鍵・失効は gateway ごとの local file で、複数 gateway・複数 host の同期、鍵 rotation の移行（再暗号化）、容量上限・GC（期限切れ file の削除）は無い。同じ snapshot の clone は memory の page cache を共有する（同一 tenant 内の side channel）。snapshot の source 起動は admission の容量予約を通らない（operator 操作）。

20. **invocation log の file（ADR-0018）。** `<data_dir>/logs/logs.db`（と `-wal` / `-shm`）は user code の stdout / stderr を平文で持つ。user code が自分の secret や利用者の個人データを出力すれば、それもそのまま入る（host は secret 値を書かない: `secret_values_reach_the_log_store_only_when_user_code_prints_them`）。gateway は `logs/` を 0700、`logs.db` を 0600 で新規作成し（`crates/application/src/logs/tests.rs::the_directory_and_the_database_are_private`）、既存の directory / file の mode は変えない。全行に tenant を持ち、読み取りは台帳で invocation の tenant を確かめたうえで store の query も tenant で絞る（`another_tenant_never_reads_the_lines`）。保持期限（既定 7 日）と総量上限で消した行は `secure_delete` で page から上書きされるが、checkpoint 前の WAL と file system の snapshot / backup には残る。保存時暗号化は無い。`/readyz` と `/metrics` の `logs` は件数と状態だけで、本文と tenant を含まない。

16. **非同期 dispatcher の at-least-once と retry（PLT-4640）。** 非同期 invocation の実行は at-least-once: handler の外部副作用の後、結果を台帳に確定する前に gateway が止まる（SIGKILL、host 障害）と、claim の期限の後に次の試行が handler をもう一度実行する（E2E `crash.before_commit.*` で 2 回実行を実測）。台帳は terminal の記録と dead letter を 1 つに保つが、外部の副作用は利用者が業務の冪等キーで 1 回にする（`docs/api.md` §5.6.3、`examples/idempotent-async`）。retry budget と非同期の並列上限は gateway process ごとで tenant ごとではない。retry の予約は outbox の件数上限に数える。dead letter・redrive 記録・inline 入力に保持期限が無い。handler エラーはすべて retry の対象で、error type ごとに retry しない設定は無い。非同期の優先度は並列上限と短い admission 待ちで表しているだけで、同じ待ち行列の中で同期を先に出す仕組みは無い。実測は macOS arm64 の 1 host・process provider・1 回（`docs/evidence/async-dispatch-e2e-*`）で、Firecracker・Linux・複数 gateway process の E2E は無い。
18. **予算は仮料金の範囲の、local な上限でしかない（PLT-4643）。** 金額は PLT-4642 の仮料金（課金区間・要求資源・転送 bytes・invocation 数）だけで、host の原価・複数 region・決済は対象外。予約・totals は `<data_dir>/usage/budget.db` の SQLite で、複製・署名・hash chain が無く、root は書き換えられる（T50）。`data_dir` を共有しない gateway 同士は予算を共有しない。最大料金は短い処理にも timeout と最大 response size の分を予約するので、上限が小さいと実消費よりずっと少ない並列数で止まり、精算（collector 間隔 + 精算）まで返らない。見積りを超えた実行（overrun）は数えるが止めず、確約は overrun の分だけ上限を超えうる（T53）。終わりを報告しない run（kill -9）は deadline + grace まで最大額を予約し、その後も最大額を unmetered hold として期間の終わりまで持つ（実際より多く数える側）。collector の停止は `max_unsettled_age_seconds` の後に全 tenant の新規受付を止める（T52、T54）。期間は UTC の暦月だけで、月をまたぐ run は予約した月に数える。非同期 invoke の受付（202）は予算を見ず、予算で拒否された run は defer され続け、最大 age で dead letter になる（ADR-0016「結果」）。Firecracker 実機での遅延は未計測。
