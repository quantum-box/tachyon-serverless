...
| A5 ledger（Function / Revision / Alias / Invocation / Attempt / Environment / Lease / UsageEvent） | `<data_dir>/state.db`（埋め込み SQLite。log は memory）。UsageEvent は `<data_dir>/usage/journal.db`（未回収）と `<data_dir>/usage/ledger.db`（回収済み、PLT-4642） | 偽の結果による上書き、guest 申告に基づく計測、usage の改竄・削除・二重計上・他 tenant への漏洩 |
