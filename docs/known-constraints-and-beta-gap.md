...
| store 停止が lease（既定 30 s + skew 2 s）を超える | 停止中は heartbeat が書けず新規の書込みは 503。**2026-09-18 から** gateway 自身が止まっていなければ fence しない | 解放後、誰にも回収されていない lease を取り戻し再起動なしで復帰（`db_locked_past_lease`、process 実測）。gateway 自身が止まっていた（SIGSTOP 等）場合は従来どおり fence → 再起動 | なし（実測） | `db_locked_past_lease`、`stale_owner_frozen_in_transaction`、[failure-matrix.md](failure-matrix.md) §6.2 |
...
| fence 後の自動復帰 | (a) supervisor が `dispatcher.fenced` で再起動 / (b) 誰にも回収されていない lease の再取得（**2026-09-18 に store 停止の場合だけ実装**、ADR-0003。process 自身が止まっていた場合は (a)） | (b) の安全性は store の直列化に依存。1 つの SQLite を共有する構成で transaction の中で止まった process は lock を持ち続ける | [failure-matrix.md](failure-matrix.md) §6.2 |
