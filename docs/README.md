...
| [adr/0014-cron-and-webhook-triggers.md](adr/0014-cron-and-webhook-triggers.md) | cron と署名付き webhook の trigger。既存 CronJob（tachyon-apps）の調査、fire 行を `invokeAsync` の受付トランザクションに入れる一意キー、time zone と DST（存在しない時刻は skip・2 回ある時刻は 1 回）、missed-run policy と catch-up window、scheduler lease、HMAC の検証順序と再送 dedup、secret の封印、disable / delete と受付済みの扱い（PLT-4641） |
| [adr/0016-budget-reservation-and-admission.md](adr/0016-budget-reservation-and-admission.md) | 予算（UTC 暦月、tenant / function、alert と停止は別設定）を設定 cache で配信し、実行前に最大料金を CAS で予約、ledger の実測で精算・過剰分を返却、終わりを報告しない run は最大額を hold。予算不明・collector 停止・store 停止は fail closed、拒否理由は quota / budget / capacity で別（PLT-4643） |
| [adr/0018-durable-invocation-logs.md](adr/0018-durable-invocation-logs.md) | invocation log を台帳と別の `<data_dir>/logs/logs.db` に、上限付き queue と writer thread の batch commit で書く（crash で失うのは直近 1 flush 間隔まで）。invocation / attempt 上限と marker、保持期限（既定 7 日）と総量上限（既定 1 GiB、実行中の invocation は消さない）、queue 満杯・lock 時は捨てて数え invocation は止めない、`/readyz` の `logs` は readiness に入れない、全 query を tenant で絞る（PLT-4628 follow-up） |
...
| [evidence/lab-20260917T1219Z-process-clean-clone/](evidence/lab-20260917T1219Z-process-clean-clone/) | `scripts/lab/lab.sh` を GitHub から fresh clone した directory で runbook どおりに実行（**自動化 agent による追試、別の人間ではない**）。process provider、macOS arm64。command log、§6 の復旧 drill、teardown の孤児検査（PLT-4648） | demo 47/47、drill 5/5、orphan check clean |
