...
5. **P1 の `Capabilities` は保守的に返す。** `snapshot_create` / `snapshot_clone` / `egress_restricted` / `egress_public_web` は `Unsupported{reason}`。`idle_quiesce` / `idle_resume` は PLT-4633 で実装し（`PATCH /vm {state: Paused/Resumed}`、上表「pause / resume」）、`scripts/kvm/measure-warm.sh`（`docs/kvm.md` §3.7）で実機計測を取った上で `Supported` に上げた。証跡は `docs/evidence/warm-20260916T162532Z/`（6 invocation 中 5 が warm、同一環境を 6 epoch 再利用、`resume_ms` / `readiness_ms` の中央値 9、休止中の VMM は 3 秒で CPU tick 0・RSS 38 MiB 据え置き）。aarch64 の nested virtualization での記録であり、x86_64 と bare metal、および再開拒否・無応答 guest といった失敗経路はテストでのみ確認している。`Supported` は `docs/evidence/` に証跡がある項目にだけ付ける。2026-09-17 時点で Firecracker provider が `Supported` を返すのは `create_terminate` / `observe` / `enforce_deadline` / `egress_none` / `idle_quiesce` / `idle_resume` で、証跡は `docs/evidence/kvm-20260915T080221Z/`（fc-smoke）、`docs/evidence/20260915T171631Z-firecracker/`（E2E 28/28）、`docs/evidence/isolation-20260916T020934Z/`（M8）、`docs/evidence/warm-20260916T162532Z/`（warm 再利用）。いずれも aarch64 の nested virtualization での記録で、x86_64 と bare metal は未測定。`host_metering` / `enforce_resource_limits` は測定が済むまで `Unverified{note}`。
