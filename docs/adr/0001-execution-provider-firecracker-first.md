...
| 起動時間（公称） | upstream サイトは「as little as 125 ms」と掲げる（[firecracker-microvm.github.io](https://firecracker-microvm.github.io/)）。本リポジトリでは aarch64 の nested virtualization・jailer・500 m の cgroup 下で cold の `environment_boot_ms` 中央値 5.1 s（`docs/benchmark.md` §6。bare metal / x86_64 は未測定） | README に具体的な公称値の明記なし。本リポジトリ未測定 | upstream 文書に headline の数値なし。VM 起動 + kata-agent + shim の合計。本リポジトリ未測定 |
...
5. **P1 の `Capabilities` は保守的に返す。** `snapshot_create` / `snapshot_clone` は `Unsupported{reason}`。`egress_restricted` / `egress_public_web` は PLT-4622 で tap + nftables により実装し（`docs/adr/0005-egress-profiles.md`）、`scripts/kvm/measure-isolation.sh` の NET（`docs/evidence/isolation-20260917T031126Z/`）を経て `Supported` にした。ただし `CAP_NET_ADMIN`・nftables・`ip_forward=1` の無い host では provider が理由付きの `Unsupported` を返す。`idle_quiesce` / `idle_resume` は PLT-4633 で実装し（`PATCH /vm {state: Paused/Resumed}`、上表「pause / resume」）、`scripts/kvm/measure-warm.sh`（`docs/kvm.md` §3.7）で実機計測を取った上で `Supported` に上げた。証跡は `docs/evidence/warm-20260916T162532Z/`（6 invocation 中 5 が warm、同一環境を 6 epoch 再利用、`resume_ms` / `readiness_ms` の中央値 9、休止中の VMM は 3 秒で CPU tick 0・RSS 38 MiB 据え置き）。aarch64 の nested virtualization での記録であり、x86_64 と bare metal、および再開拒否・無応答 guest といった失敗経路はテストでのみ確認している。`Supported` は `docs/evidence/` に証跡がある項目にだけ付ける。2026-09-17 時点で Firecracker provider が `Supported` を返すのは `create_terminate` / `observe` / `enforce_deadline` / `egress_none` / `egress_restricted` / `egress_public_web`（host が強制できる場合）/ `enforce_resource_limits` / `idle_quiesce` / `idle_resume` で、証跡は `docs/evidence/kvm-20260915T080221Z/`（fc-smoke）、`docs/evidence/20260915T171631Z-firecracker/`（E2E 28/28）、`docs/evidence/isolation-20260916T020934Z/`（M8）、`docs/evidence/isolation-20260917T011555Z/`（M8・M9 の再計測と ephemeral storage。PLT-4622 で `enforce_resource_limits` を昇格。上限は vCPU 数・`mem_size_mib`・scratch drive の大きさで、VMM への host 側 cgroup は無い）、`docs/evidence/warm-20260916T162532Z/`（warm 再利用）、`docs/evidence/isolation-20260917T031126Z/`（egress restricted / public-web、2 tenant 同時、初期 race、後始末）。いずれも aarch64 の nested virtualization での記録で、x86_64 と bare metal は未測定。`host_metering` は測定が済むまで `Unverified{note}`。
...
| # | 測定 | 合格の目安（数値は仮置き。測定後に更新） | 状況（2026-09-17） |
|---|---|---|---|
...
| M2 | `environment_boot_ms`（create 開始 → bridge の `Hello` 受信）、N ≥ 20 | 中央値と p95 を記録。目安は無い（初回は事実の記録） | 実測済み（aarch64 nested、N = 20、jailer + cgroup required。PLT-4647 `scripts/kvm/bench.sh`、`docs/evidence/bench-20260917T055450Z/`）: cold の中央値 / p95 は hello 5102 / 5315 ms、http-axum 5551 / 7776 ms、cpu-burn 5756 / 10490 ms。起動中の cgroup throttled period 比 p50 0.99（500 m quota）。fresh host（page cache なし、n = 5）は hello 5438 / 6836 ms / 未測定（x86_64、bare metal） |
| M3 | `runtime_init_ms`（`Hello` → `Ready`）hello サンプル | 同上 | 実測済み（M2 と同じ記録）: cold の中央値 / p95 は hello 501 / 566 ms、http-axum 555 / 819 ms、cpu-burn 499 / 1120 ms / 未測定（x86_64、bare metal） |
| M4 | `handler_ms` / `total_ms` hello・http-axum | 同上 | 実測済み（M2 と同じ記録、N = 20）: cold の `handler_ms` / `total_ms` の中央値（p95）は hello 112（173）/ 5729（5935）ms、http-axum 122（215）/ 6365（9154）ms。warm（`[pool] enabled`）は hello 42（48）/ 49（68）ms、http-axum 47（102）/ 56（107）ms。client から見た値と失敗を含む分布は `docs/benchmark.md` §6 / 未測定（x86_64、bare metal） |
...
| M7 | orphan 検査: N 回の invoke 後に firecracker プロセス、uds、drive、workdir が残らない | 0 件 | 実測済み（aarch64 nested）: E2E 1 回分（`docs/evidence/20260915T125610Z-firecracker/` step 19・27）に加え、PLT-4647 の 714 request・370 環境（gateway 18 回の起動停止、jailer + cgroup）の後に firecracker / jailer / gateway プロセス、環境 cgroup、jail、tap、nft table、環境ディレクトリがすべて 0（`docs/evidence/bench-20260917T055450Z/` `cleanup.txt`） / 未測定（x86_64） |
...
| M10 | 同時実行: `max_concurrency = 4` で 4 並列、5 本目が queue に入る | 429 / 504 の分類が仕様どおり | 一部実測済み（PLT-4647 の sweep: revision `max_concurrency = 4` で並列 8 のとき queue に入り、queue deadline 10 s を超えた分が 504 `Host.QueueTimeout`、429 は 0。`docs/evidence/bench-20260917T055450Z/`） / 未測定（`max_queue` 溢れの 429） |
