...
| pause / resume | unverified（upstream は `PATCH /vm {state: Paused/Resumed}` を提供）。PLT-4633 で実装したので `Capabilities.idle_quiesce/idle_resume` は `Unverified`（実機未計測） | unverified（upstream は `vm.pause` / `vm.resume` を提供） | unsupported（Kubernetes Pod API に pause は無く、RuntimeClass 経由で露出しない） |
