...
| ephemeral storage 上限 | verified（PLT-4622。`/tmp` は `ephemeral_storage_mib` の scratch drive で、64 MiB の環境で 58 MiB 書いて `ENOSPC`。`docs/evidence/isolation-20260917T011555Z/`、aarch64 nested） | unverified | unverified（`ephemeral-storage` request/limit を宣言。ADR-0025 は `local-path` driver が claim ごとの bytes を強制しないと記録） |
| pause / resume | verified（upstream の `PATCH /vm {state: Paused/Resumed}`）。PLT-4633 で実装し実機計測を取ったので `Capabilities.idle_quiesce/idle_resume` は `Supported`（証跡 `docs/evidence/warm-20260916T162532Z/`） | unverified（upstream は `vm.pause` / `vm.resume` を提供） | unsupported（Kubernetes Pod API に pause は無く、RuntimeClass 経由で露出しない） |
...
`Capabilities`（`crates/provider-port/src/execution.rs`）では、Firecracker provider は `snapshot_create` / `snapshot_clone` / `egress_restricted` / `egress_public_web` を `Unsupported`、`idle_quiesce` / `idle_resume` を `Supported`（PLT-4633 で実装し `docs/kvm.md` §3.7 で実機計測。証跡 `docs/evidence/warm-20260916T162532Z/`）として返す（`docs/architecture.md` §1, §4, §6）。上表の `unverified` は「後続で測定可能」の意味であり、P1 で提供する意味ではない。Firecracker provider の実装は `create_terminate` / `observe` / `enforce_deadline` / `egress_none` / `enforce_resource_limits` / `idle_quiesce` / `idle_resume` を `Supported` で返す。根拠は本表の verified 記録（aarch64 の nested virtualization）で、x86_64 と bare metal は未測定。`egress_none` は ADR-0001 M8 を実機で測ったので `Supported`（`docs/evidence/isolation-20260916T020934Z/`）、`enforce_resource_limits` は M9 と ephemeral storage を実機で測ったので PLT-4622 で `Supported`（`docs/evidence/isolation-20260917T011555Z/`）、`host_metering` は `Unverified` で返す。このうち上表で `verified` にしたのは create / terminate と deadline だけで、どちらも baseline profile 外の記録である。
