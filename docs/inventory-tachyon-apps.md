...
`Capabilities`（`crates/provider-port/src/execution.rs`）では、Firecracker provider は `snapshot_create` / `snapshot_clone` / `egress_restricted` / `egress_public_web` を `Unsupported`、`idle_quiesce` / `idle_resume` を `Supported`（PLT-4633 で実装し `docs/kvm.md` §3.7 で実機計測。証跡 `docs/evidence/warm-20260916T162532Z/`）として返す（`docs/architecture.md` §1, §4, §6）。上表の `unverified` は「後続で測定可能」の意味であり、P1 で提供する意味ではない。Firecracker provider の実装は `create_terminate` / `observe` / `enforce_deadline` / `egress_none` / `idle_quiesce` / `idle_resume` を `Supported` で返す。根拠は本表の verified 記録（aarch64 の nested virtualization）で、x86_64 と bare metal は未測定。`egress_none` は ADR-0001 M8 を実機で測ったので `Supported`（`docs/evidence/isolation-20260916T020934Z/`）、`host_metering` / `enforce_resource_limits` は `Unverified` で返す。このうち上表で `verified` にしたのは create / terminate と deadline だけで、どちらも baseline profile 外の記録である。
